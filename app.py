...
# --- 2. 披薩指數核心技術：導航欄局部擷取 + 強化辨識 ---

def get_pizza_intel_pro():
    """
    技術層次：3x 影像放大 + 二值化強化 + 寬鬆 Regex 補漏
    """
    lvl, pct = 1, 0.0
    raw_debug_text = ""
    
    status = st.status("🍕 正在實施披薩技術偵察...", expanded=True)
    try:
        with sync_playwright() as p:
            status.write("1. 進入 WorldMonitor 衛星站...")
            browser = p.chromium.launch(headless=True, args=['--no-sandbox', '--disable-gpu'])
            page = browser.new_page(viewport={'width': 1920, 'height': 1080})
            page.goto("https://worldmonitor.app/", wait_until="domcontentloaded", timeout=60000)
            time.sleep(6) # 延長等待確保數據完全渲染
            
            status.write("2. 執行局部擷取與影像強化...")
            # 針對導航欄右側數據區實施精準裁切
            screenshot = page.screenshot(clip={'x': 1000, 'y': 0, 'width': 920, 'height': 120})
            browser.close()
            ...
            status.write("3. 執行多重 Regex 數據提取...")
            # 寬鬆 Regex：捕捉 DEFCON 1 或 類似變體
            lvl_m = re.search(r'(?:defcon|d\w+con|gd|d)\s*[:|l|!|i]?\s*(\d)', raw_debug_text, re.IGNORECASE)
            # 捕捉百分比數字 (例如 59%)
            ...
            status.update(label="✅ 偵察完成", state="complete", expanded=False)
...
def fetch_vixtwn_physical():
    ...
    try:
        with sync_playwright() as p:
            ...
            time.sleep(5)
            # 點擊截圖 1000028179.jpg 中的橘色按鈕位置
            page.mouse.click(465, 960) 
            page.evaluate("""() => { const b = Array.from(document.querySelectorAll('button, a')).find(x => x.innerText.includes('接受') || x.className.includes('orange')); if(b) b.click(); }""")
            time.sleep(8)
...
if st.button("🛰️ 執行披薩技術更新", use_container_width=True):
...
st.markdown(f"""
    <div style="background-color:#000; border-radius:12px; padding:20px; border:1px solid #333; text-align:center;">
        ...
        <p style="font-size:10px; color:#666; margin-top:10px;">最後更新：{saved_p['time']}</p>
    </div>
""", unsafe_allow_html=True)

with st.expander("🕵️ 查看 OCR 偵察日誌"):
